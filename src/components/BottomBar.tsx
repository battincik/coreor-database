...
export default function BottomBar({ selectedDatabase, selectedTable }: BottomBarProps) {
  ...
  return <div className="shrink-0 border-t border-zinc-800 bg-zinc-950 text-xs">
    ...
    <div className="coreor-hide-scrollbar h-7 overflow-x-auto border-t border-zinc-800 text-[9px] text-zinc-500"><div className="flex h-full min-w-max items-center divide-x divide-zinc-800 tabular-nums">
      ...
      <StatusTooltip title="Sunucu ağ trafiği" description="Motor açılışından beri alınan ve gönderilen toplam bayt sayaçları." rows={[{label:'Alınan',value:serverSnapshot?formatBytes(serverSnapshot.bytesReceived):'—'},{label:'Gönderilen',value:serverSnapshot?formatBytes(serverSnapshot.bytesSent):'—'},{label:'Toplam',value:serverSnapshot?formatBytes(serverSnapshot.bytesReceived+serverSnapshot.bytesSent):'—'},{label:'Dönem',value:'Sunucu açılışından beri'}]}><span className="flex h-full items-center gap-1.5 px-2"><Network className="h-3 w-3 text-cyan-400"/><span><ArrowDown className="inline h-2.5 w-2.5 text-emerald-400"/> {serverSnapshot?formatBytes(serverSnapshot.bytesReceived):'—'}</span><span><ArrowUp className="inline h-2.5 w-2.5 text-blue-400"/> {serverSnapshot?formatBytes(serverSnapshot.bytesSent):'—'}</span></span></StatusTooltip>
      <StatusTooltip title="SQL başarı ve süreleri" description="Yalnızca kullanıcının yaptığı ve SQL günlüğüne kaydedilen işlemler." align="right" rows={[{label:'Toplam',value:queryEntries.length},{label:'Başarılı',value:metrics.successful,tone:'success'},{label:'Hata',value:metrics.failed,tone:metrics.failed?'danger':'normal'},{label:'Uyarı',value:metrics.warnings},{label:'Başarı oranı',value:`%${metrics.successRate}`},{label:'Ortalama süre',value:metrics.averageDuration===undefined?'—':`${metrics.averageDuration} ms`}]}><span className="flex h-full items-center gap-1 px-2"><Terminal className="h-3 w-3"/>{queryEntries.length} SQL • %{metrics.successRate}</span></StatusTooltip>
