...
export default function BottomBar({selectedDatabase,selectedTable}:BottomBarProps){
  ...
  return <div className="shrink-0 border-t border-zinc-800 bg-zinc-950 text-xs">
    ...
    <div className="flex h-8 border-t border-zinc-800 text-[9px] text-zinc-500"><div className="coreor-hide-scrollbar min-w-0 flex-1 overflow-x-auto"><div className="flex h-full min-w-max items-center divide-x divide-zinc-800">
      <Metric title="Bağlantı durumu" description="Son sağlık kontrolü ve aktif bağlantı profili." rows={[{label:t('maintenance.status'),value:!activeServer?t('bottomBar.serverNotSelected'):snapshotError?t('bottomBar.healthLimited'):t('common.connected'),tone:snapshotError?'warning':'success'},{label:t('bottomBar.engine'),value:activeServer?databaseEngineLabel(activeServer.databaseType):'—'},{label:'Son kontrol',value:snapshot?.sampledAt?formatDate(snapshot.sampledAt):'—'},{label:t('common.error'),value:snapshotError||t('bottomBar.none')}]} onTooltip={setTooltip}>{loading?<Loader2 className="h-3 w-3 animate-spin"/>:<Wifi className={`h-3 w-3 ${activeServer?'text-emerald-400':'text-zinc-600'}`}/>} {activeServer?t('common.connected'):t('topbar.noConnection')}</Metric>
      <Metric title="Sunucu profili" description="Motor, sürüm, host ve güvenlik ayarları." rows={[{label:t('server.profile'),value:activeServer?.name||'—'},{label:t('bottomBar.engine'),value:definition.label},{label:'Host',value:activeServer?`${activeServer.host}:${activeServer.port}`:'—'},{label:t('bottomBar.user'),value:activeServer?.username||'—'},{label:'TLS',value:activeServer?.sslMode||'—'}]} onTooltip={setTooltip}><Server className="h-3 w-3"/> {definition.label}</Metric>
      <Metric title="Sunucu uptime" description="Veritabanı motorunun yeniden başlatılmadan çalıştığı süre." rows={[{label:t('bottomBar.uptime'),value:snapshot?formatDuration(snapshot.uptimeSeconds,true):'—'},{label:t('bottomBar.totalQueries'),value:snapshot?.questions.toLocaleString(language)||'—'},{label:t('bottomBar.slowQueries'),value:snapshot?.slowQueries.toLocaleString(language)||'—',tone:snapshot?.slowQueries?'warning':'normal'}]} onTooltip={setTooltip}><Timer className="h-3 w-3 text-cyan-400"/> {snapshot?formatDuration(snapshot.uptimeSeconds):t('bottomBar.uptimeEmpty')}</Metric>
      <Metric title="Coreor bağlantı süresi" description="Aktif sunucu profilinin bu uygulama oturumunda seçili kaldığı süre; sürekli açık DB bağlantısı değildir." rows={[{label:t('maintenance.duration'),value:activeServer?formatDuration(connectionSeconds,true):'—'},{label:t('bottomBar.startedAt'),value:startedAt?formatDate(startedAt):'—'},{label:t('server.profile'),value:activeServer?.name||'—'}]} onTooltip={setTooltip}><PlugZap className="h-3 w-3 text-emerald-400"/> {activeServer?formatDuration(connectionSeconds):'—'}</Metric>
      <Metric title="Aktif hedef" description="Sorgu ve tablo işlemlerinin mevcut kapsamı." rows={[{label:t('database.database'),value:target},{label:t('database.table'),value:selectedTable||'—'},{label:t('bottomBar.catalogDatabases'),value:activeServer?.databases?.length||0},{label:t('bottomBar.catalogTables'),value:activeServer?.databases?.reduce((sum,item)=>sum+item.tableCount,0)||0}]} onTooltip={setTooltip}><Database className="h-3 w-3"/> {target}{selectedTable?` / ${selectedTable}`:''}</Metric>
      <Metric title="Grid durumu" description="Aktif tablo görünümündeki sayfalama, filtre ve sıralama." rows={[{label:t('bottomBar.page'),value:`${grid.page}/${grid.totalPages}`},{label:t('bottomBar.pageSize'),value:grid.pageSize},{label:t('bottomBar.totalRows'),value:grid.totalRows.toLocaleString(language)},{label:t('bottomBar.filter'),value:grid.filters},{label:t('bottomBar.sorting'),value:grid.sorts},{label:t('maintenance.status'),value:grid.isLoading?t('common.loading'):t('common.ready')}]} onTooltip={setTooltip}><Table className="h-3 w-3"/> {grid.page}/{grid.totalPages} • {grid.pageSize}</Metric>
      <Metric title="Bağlantılar ve threadler" description="MySQL/MariaDB bağlantı havuzu ve aktif çalışma bilgileri." rows={[{label:t('common.connected'),value:snapshot?.threadsConnected||'—'},{label:t('bottomBar.running'),value:snapshot?.threadsRunning||'—'},{label:t('bottomBar.peak'),value:snapshot?.maxUsedConnections||'—'},{label:'Maksimum',value:snapshot?.maxConnections||'—'},{label:'Reddedilen',value:snapshot?.abortedConnects||'—'}]} onTooltip={setTooltip}><Activity className="h-3 w-3 text-purple-400"/> {snapshot?`${snapshot.threadsConnected} bağlı • ${snapshot.threadsRunning} çalışan`:'Thread —'}</Metric>
      <Metric title="InnoDB buffer pool" description="Bellek havuzu doluluk, dirty page ve hit ratio değerleri." rows={[{label:t('bottomBar.usage'),value:snapshot?`%${snapshot.bufferPool.usagePercent.toFixed(1)}`:'—'},{label:'Dirty',value:snapshot?`%${snapshot.bufferPool.dirtyPercent.toFixed(1)}`:'—'},{label:'Hit ratio',value:snapshot?.bufferPool.hitRatio==null?'—':`%${snapshot.bufferPool.hitRatio.toFixed(2)}`},{label:t('bottomBar.total'),value:snapshot?formatBytes(snapshot.bufferPool.totalPages*snapshot.bufferPool.pageSize):'—'}]} onTooltip={setTooltip}><Gauge className="h-3 w-3 text-amber-400"/> Buffer {snapshot?`%${Math.round(snapshot.bufferPool.usagePercent)}`:'—'}</Metric>
      <Metric title="Depolama" description={selectedDatabase?'Seçili veritabanının veri + indeks boyutu. Sunucu toplamı ve InnoDB ayrılmış boş alanı ayrıntılarda gösterilir.':'Katalog/performance metadata üzerinden hesaplanan veritabanı veri + indeks toplamı. Fiziksel disk kapasitesi değildir.'} rows={[
        ...
        {label:'Kaynak',value:snapshot?t('bottomBar.liveMetadata'):'Katalog cache'}
      ...
      <Metric title="Sunucu ağ trafiği" description="Motor açılışından beri alınan ve gönderilen toplam bayt." rows={[{label:t('bottomBar.received'),value:snapshot?formatBytes(snapshot.bytesReceived):'—'},{label:t('bottomBar.sent'),value:snapshot?formatBytes(snapshot.bytesSent):'—'},{label:t('bottomBar.total'),value:snapshot?formatBytes(snapshot.bytesReceived+snapshot.bytesSent):'—'}]} onTooltip={setTooltip}><Network className="h-3 w-3 text-cyan-400"/><ArrowDown className="h-2.5 w-2.5 text-emerald-400"/>{snapshot?formatBytes(snapshot.bytesReceived):'—'}<ArrowUp className="h-2.5 w-2.5 text-blue-400"/>{snapshot?formatBytes(snapshot.bytesSent):'—'}</Metric>
      <Metric title="SQL istatistikleri" description="Bu uygulama oturumundaki kullanıcı sorgularının başarı ve süre özeti." rows={[{label:t('bottomBar.total'),value:entries.length},{label:t('common.success'),value:stats.successful,tone:'success'},{label:t('common.error'),value:stats.errors,tone:stats.errors?'danger':'normal'},{label:t('bottomBar.successRate'),value:`%${stats.rate}`},{label:t('bottomBar.averageDuration'),value:`${stats.average} ms`},{label:t('bottomBar.lastQuery'),value:stats.last?.durationMs==null?'—':`${stats.last.durationMs} ms`}]} onTooltip={setTooltip}><Terminal className="h-3 w-3"/> {entries.length} SQL • %{stats.rate}</Metric>
      <Metric title="Filtre ve sıralama" description="Aktif veri gridindeki koşul ve sıralama sayısı." rows={[{label:t('bottomBar.filter'),value:grid.filters},{label:t('bottomBar.sorting'),value:grid.sorts},{label:t('bottomBar.loadingState'),value:grid.isLoading?'Devam ediyor':'Beklemede'}]} onTooltip={setTooltip}><Filter className="h-3 w-3"/> {grid.filters}<ArrowUpDown className="h-3 w-3"/> {grid.sorts}</Metric>
      <Metric title="Son sorgu" description="En son çalıştırılan kullanıcı SQL işlemi." rows={[{label:'Zaman',value:stats.last?formatDate(stats.last.timestamp):'—'},{label:t('statusGuide.server'),value:stats.last?.serverName||'—'},{label:t('server.target'),value:stats.last?.databaseName||t('query.serverScope')},{label:t('maintenance.duration'),value:stats.last?.durationMs==null?'—':`${stats.last.durationMs} ms`},{label:t('query.rows'),value:stats.last?.rowCount??stats.last?.affectedRows??'—'}]} onTooltip={setTooltip}><Clock className="h-3 w-3"/> {stats.last?.durationMs==null?t('bottomBar.lastQueryEmpty'):`${stats.last.durationMs} ms`}</Metric>
