    ...
    <div className="fixed inset-0 z-[260] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5 shadow-2xl">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1"><h3 className="text-sm font-semibold text-zinc-100">{state.title}</h3><p className="mt-1 text-xs leading-5 text-zinc-500">{state.description}</p></div>
        </div>
...
export function TableSchemaEditor({
  ...
  useEffect(() => {
    workingTableNameRef.current = tableName;
  }, [tableName]);
  ...
  const addColumn = () => setColumns(previous => [...previous, {
  ...
  const moveColumn = async (index: number, direction: -1 | 1) => {
    ...
    try {
      await runMutation({ kind: 'modify-column', originalName: moved.originalName || moved.name, column: cleanColumnDefinition(moved), first: target === 0, after: target === 0 ? null : previous?.name || null }, `${moved.name} kolonu taşındı.`);
    } catch {
      setColumns(info.columns.map(columnToDraft));
    }
  };
  ...
  const addIndex = async () => {
    try {
      await runMutation({ kind: 'add-index', index: { kind: indexDraft.kind, name: indexDraft.kind === 'PRIMARY' ? undefined : indexDraft.name, columns: indexDraft.columns.map(name => ({ name })), comment: indexDraft.comment } }, 'İndeks eklendi.');
      clearIndexDraft();
    } catch {
      // handled
    }
  ...
  const resetForeignKeyDraft = () => {
    setEditingForeignKeyName(null);
  ...
  const editForeignKey = (name: string, rows: typeof info.foreignKeys) => {
    ...
    setForeignKeyDraft({
      name,
      columns: orderedRows.map(row => row.COLUMN_NAME),
      referencedDatabase: orderedRows[0]?.REFERENCED_TABLE_SCHEMA || databaseName,
      referencedTable: orderedRows[0]?.REFERENCED_TABLE_NAME || '',
      referencedColumns: orderedRows.map(row => row.REFERENCED_COLUMN_NAME),
      onDelete: (orderedRows[0]?.DELETE_RULE || 'RESTRICT') as TableForeignKeyDefinition['onDelete'],
      onUpdate: (orderedRows[0]?.UPDATE_RULE || 'RESTRICT') as TableForeignKeyDefinition['onUpdate']
    });
  };

  const saveForeignKey = async () => {
    try {
      if (editingForeignKeyName) await runMutation({ kind: 'drop-foreign-key', constraintName: editingForeignKeyName }, `${editingForeignKeyName} foreign key kuralı kaldırıldı.`);
      await runMutation({ kind: 'add-foreign-key', foreignKey: foreignKeyDraft }, editingForeignKeyName ? 'Foreign key güncellendi.' : 'Foreign key eklendi.');
      resetForeignKeyDraft();
    } catch {
      // handled
    }
  };
  ...
  const dropForeignKey = (name: string) => setConfirm({
    ...
    onConfirm: async () => {
      await runMutation({ kind: 'drop-foreign-key', constraintName: name }, `${name} foreign key bağlantısı silindi.`);
      if (editingForeignKeyName === name) resetForeignKeyDraft();
    }
  });
    ...
    <div className="flex h-full min-h-0 flex-col bg-[#0b0d0f]">
      <Tabs defaultValue="basic" className="flex min-h-0 flex-1 flex-col">
            ...
            <div className="min-w-[1600px]">
              <Table size="sm" className="w-full table-fixed text-[10px]">
                ...
                <TableBody>{columns.map((column, index) => <TableRow key={column.id} className={column.dirty || column.isNew ? 'bg-cyan-500/[0.04]' : ''}>
                  ...
                  <TableCell className="border p-0.5"><select value={column.dataType} onChange={event => updateColumn(column.id, { dataType: event.target.value })} className={`${CONTROL_CLASS} w-full text-green-400`}>{DATA_TYPES.map(type => <option key={type}>{type}</option>)}</select></TableCell>
        ...
        <TabsContent value="indexes" className="m-0 flex min-h-0 flex-1 flex-col overflow-hidden p-0">
          <div className="grid shrink-0 gap-2 border-b border-zinc-800 p-2 md:grid-cols-[120px_160px_minmax(260px,1fr)_minmax(180px,0.7fr)_90px_80px]">
            ...
            <Button type="button" variant="ghost" size="sm" className="h-7 text-[10px]" onClick={clearIndexDraft}>Temizle</Button>
          </div>
        ...
        <TabsContent value="foreign-keys" className="m-0 flex min-h-0 flex-1 flex-col overflow-hidden p-0">
          <div className="grid shrink-0 gap-2 border-b border-zinc-800 p-2 lg:grid-cols-[150px_minmax(180px,1fr)_150px_150px_minmax(180px,1fr)_110px_110px_90px_80px]">
