...
function Sparkline({ values }: { values: number[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-10 w-full" preserveAspectRatio="none" aria-hidden="true">
      <polyline points={points} fill="none" stroke="currentColor" strokeWidth="2" vectorEffect="non-scaling-stroke" className="text-cyan-400" />
    </svg>
  );
}
...
function MetricCard({ title, value, description, icon, values, tone = 'cyan' }: {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-zinc-800 bg-black/20 p-3">
      <div className="flex items-center gap-2 text-[10px] font-medium uppercase tracking-[0.12em] text-zinc-600"><span className={toneClass}>{icon}</span>{title}</div>
      <div className="mt-2 text-2xl font-semibold tabular-nums text-zinc-100">{value}</div>
      <div className="mt-1 min-h-8 text-[10px] leading-4 text-zinc-500">{description}</div>
      {values && <div className={`mt-2 ${toneClass}`}><Sparkline values={values} /></div>}
    </div>
  );
}
...
export function DatabasePerformancePanelModal({ open, onClose, serverId, accountId, selectedDatabase }: DatabasePerformancePanelModalProps) {
  ...
  const load = useCallback(async () => {
    if (!serverId || !accountId || loading) return;
    setLoading(true);
    setError(null);
    try {
      ...
      setSnapshot(next);
      setPoints(previousPoints => [...previousPoints, point].slice(-36));
    } catch (failure) {
      setError(failure instanceof Error ? failure.message : 'Performans snapshot alınamadı.');
    } finally {
      setLoading(false);
    }
  }, [serverId, accountId, selectedDatabase, loading]);
  ...
  const chartSeries = useMemo(() => ({
    qps: points.map(point => point.qps),
    connections: points.map(point => point.connections),
    running: points.map(point => point.running),
    slow: points.map(point => point.slowPerSecond)
  }), [points]);
    ...
    <div className="fixed inset-0 z-[328] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex h-[min(880px,95vh)] w-[min(1420px,97vw)] min-h-0 flex-col overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950 shadow-2xl">
        <div className="flex h-12 shrink-0 items-center gap-3 border-b border-zinc-800 px-4">
          ...
          <div><h2 className="text-sm font-semibold">Veritabanı performans paneli</h2><p className="text-[10px] text-zinc-500">Global status, InnoDB buffer pool, replication ve mantıksal depolama metrikleri.</p></div>
          <label className="ml-auto flex items-center gap-2 text-[10px] text-zinc-500"><input type="checkbox" checked={autoRefresh} onChange={event => setAutoRefresh(event.target.checked)} />5 sn otomatik yenile</label>
        ...
        {!snapshot && loading ? <div className="flex min-h-0 flex-1 items-center justify-center gap-2 text-sm text-zinc-500"><Loader2 className="h-5 w-5 animate-spin" />Performans metrikleri okunuyor…</div> : snapshot && <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-2">
            <section className="rounded-xl border border-zinc-800 bg-black/20 p-4">
              <div className="flex items-center justify-between"><div><h3 className="text-xs font-semibold">Bağlantı ve trafik</h3><p className="mt-1 text-[10px] text-zinc-600">Son {Math.max(1, points.length)} snapshot; modal kapatılınca sıfırlanır.</p></div><div className="text-right text-[10px] text-zinc-500"><div>Alış {formatStorageBytes(latestPoint?.receivePerSecond || 0)}/sn</div><div>Gönderim {formatStorageBytes(latestPoint?.sendPerSecond || 0)}/sn</div></div></div>
