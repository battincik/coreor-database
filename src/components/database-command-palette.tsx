...
function normalized(value: string) { return value.trim().toLocaleLowerCase('tr-TR'); }
...
export function DatabaseCommandPalette({ servers, activeServerId, selectedDatabase, selectedTable, onDatabaseSelect, onTableSelect }: DatabaseCommandPaletteProps) {
  ...
  const paletteShortcut = shortcutFor(safetyWorkspace.shortcuts, 'command-palette', 'Mod+K');
  const activeServer = servers.find(server => server.id === activeServerId) ?? null;
  const mysqlWorkbench = databaseEngineFamily(activeServer?.databaseType) === 'mysql';

  useEffect(() => {
    const keyHandler = (event: KeyboardEvent) => {
      if (shortcutMatches(event, paletteShortcut)) { event.preventDefault(); setOpen(previous => !previous); }
  ...
  useEffect(() => { if (!open) return; setQuery(''); setActiveIndex(0); window.requestAnimationFrame(() => inputRef.current?.focus()); }, [open]);
  ...
  const allItems = useMemo<PaletteItem[]>(() => {
    const items: PaletteItem[] = [
      ...
      { id: 'backups', category: 'Komutlar', label: 'Yedekleme merkezi', description: 'mysqldump, pg_dump ve SQL Server görev planları', keywords: 'backup yedek mysqldump pg_dump bacpac', icon: <HardDriveDownload className="h-4 w-4" />, disabled: !activeServer, run: () => openDatabaseSafetyCenter({ tab: 'backups', serverId: activeServerId, database: selectedDatabase }) },
      { id: 'history', category: 'Komutlar', label: 'Tablo geçmişi ve migration', description: 'ALTER öncesi snapshot ve migration dosyaları', keywords: 'snapshot migration history geçmiş alter', icon: <FileClock className="h-4 w-4" />, disabled: !activeServer, run: () => openDatabaseSafetyCenter({ tab: 'history', serverId: activeServerId, database: selectedDatabase, table: selectedTable }) },
      { id: 'prepared', category: 'Komutlar', label: 'Prepared statement laboratuvarı', description: 'Parametre setlerini kaydet ve çalıştır', keywords: 'prepared parameter parametre', icon: <Braces className="h-4 w-4" />, disabled: !activeServer, run: () => openDatabaseSafetyCenter({ tab: 'prepared', serverId: activeServerId, database: selectedDatabase }) },
      { id: 'shortcuts', category: 'Komutlar', label: 'Klavye kısayolları', description: 'Global uygulama kısayollarını değiştir', keywords: 'shortcut keyboard kısayol', icon: <KeyRound className="h-4 w-4" />, run: () => openDatabaseSafetyCenter({ tab: 'shortcuts' }) },
    ...
    for (const database of activeServer?.databases || []) {
      items.push({ id: `database:${database.name}`, category: 'Veritabanları', label: database.name, description: `${database.tableCount} tablo • ${database.totalRows.toLocaleString('tr-TR')} satır`, keywords: `${database.name} database veritabanı`, icon: <Database className="h-4 w-4" />, run: () => { onDatabaseSelect(database.name); onTableSelect(null); } });
      for (const table of database.tables) items.push({ id: `table:${database.name}:${table}`, category: 'Tablolar', label: `${database.name}.${table}`, description: selectedDatabase === database.name && selectedTable === table ? 'Şu an açık tablo' : 'Tablo verisini aç', keywords: `${database.name} ${table} tablo`, icon: <Table2 className="h-4 w-4" />, run: () => { onDatabaseSelect(database.name); window.requestAnimationFrame(() => onTableSelect(table)); } });
    }
    return items;
  ...
  const execute = (item?: PaletteItem) => { if (!item || item.disabled) return; setOpen(false); item.run(); };
  if (!open || typeof document === 'undefined') return null;

  let lastCategory: PaletteItem['category'] | null = null;
  const sqlMode = query.trimStart().startsWith('>');
  return createPortal(<div className="fixed inset-0 z-[640] flex items-start justify-center px-4 pt-[10vh]"><button type="button" className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setOpen(false)} /><div className="relative z-10 flex max-h-[76vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-zinc-700 bg-zinc-950/98 shadow-2xl backdrop-blur">
    <div className={`flex h-14 shrink-0 items-center gap-3 border-b px-4 ${sqlMode ? 'border-cyan-500/25 bg-cyan-500/[0.04]' : 'border-zinc-800'}`}><Search className={`h-5 w-5 ${sqlMode ? 'text-emerald-400' : 'text-cyan-400'}`} /><input ref={inputRef} value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => { if (event.key === 'ArrowDown') { event.preventDefault(); setActiveIndex(previous => visibleItems.length ? Math.min(visibleItems.length - 1, previous + 1) : 0); } if (event.key === 'ArrowUp') { event.preventDefault(); setActiveIndex(previous => Math.max(0, previous - 1)); } if (event.key === 'Enter') { event.preventDefault(); execute(visibleItems[activeIndex]); } if (event.key === 'Escape') { event.preventDefault(); setOpen(false); } }} placeholder="> SELECT * FROM users veya komut ara…" className="min-w-0 flex-1 bg-transparent font-mono text-sm text-zinc-100 outline-none placeholder:text-zinc-600" /><span className={`rounded border px-2 py-1 text-[9px] ${sqlMode ? 'border-emerald-500/25 text-emerald-300' : 'border-zinc-800 text-zinc-500'}`}>{sqlMode ? 'SQL MODU' : displayShortcut(paletteShortcut)}</span></div>
    {sqlMode && <div className="border-b border-zinc-800 px-4 py-2 text-[9px] text-zinc-600"><span className="font-mono text-emerald-400">&gt;</span> işaretinden sonra yazılan sorgu seçili veritabanında yeni sekmede çalıştırılır.</div>}
    <div className="min-h-0 flex-1 overflow-y-auto p-2">{visibleItems.length === 0 && <div className="flex flex-col items-center justify-center px-6 py-14 text-center"><Command className="mb-3 h-8 w-8 text-zinc-700" /><div className="text-sm text-zinc-400">{sqlMode ? 'Çalıştırılacak SQL’i yazın.' : 'Eşleşen komut bulunamadı.'}</div></div>}{visibleItems.map((item, index) => { const showCategory = lastCategory !== item.category; lastCategory = item.category; return <React.Fragment key={item.id}>{showCategory && <div className="px-2 pb-1 pt-2 text-[9px] font-semibold uppercase tracking-[0.14em] text-zinc-600">{item.category}</div>}<button type="button" disabled={item.disabled} onMouseEnter={() => setActiveIndex(index)} onClick={() => execute(item)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left ${index === activeIndex ? 'bg-cyan-500/12 text-cyan-100' : 'text-zinc-300 hover:bg-white/[0.04]'} disabled:opacity-35`}><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border ${index === activeIndex ? 'border-cyan-500/30 bg-cyan-500/10 text-cyan-300' : 'border-zinc-800 bg-black/20 text-zinc-500'}`}>{item.icon}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-medium">{item.label}</span><span className="mt-0.5 block truncate text-[10px] text-zinc-600">{item.description}</span></span>{item.shortcut && <span className="rounded border border-zinc-800 px-1.5 py-0.5 text-[9px] text-zinc-600">{item.shortcut}</span>}</button></React.Fragment>; })}</div>
    <div className="flex h-9 shrink-0 items-center gap-3 border-t border-zinc-800 px-4 text-[9px] text-zinc-600"><span>↑↓ gezin</span><span>Enter çalıştır</span><span className="font-mono text-emerald-400">&gt; SQL</span><span className="ml-auto flex items-center gap-1"><Database className="h-3 w-3" />{activeServer?.name || 'Bağlantı yok'}{selectedDatabase ? ` / ${selectedDatabase}` : ''}</span></div>
