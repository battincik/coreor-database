...
export function CoreorLoadingScreen({ title, description }: { title?: string; description?: string }) {
  const {t}=useLanguage();
  const resolvedTitle=title || t('loadingScreen.title');
  const resolvedDescription=description || t('loadingScreen.description');
  return (
    <div className="relative flex h-screen min-h-[420px] items-center justify-center overflow-hidden bg-background px-6 text-foreground">
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-border/80 bg-card/75 p-8 text-center shadow-2xl backdrop-blur-xl">
        ...
        <h1 className="mt-6 text-base font-semibold tracking-tight">{resolvedTitle}</h1>
        <p className="mx-auto mt-2 max-w-sm text-xs leading-5 text-muted-foreground">{resolvedDescription}</p>
