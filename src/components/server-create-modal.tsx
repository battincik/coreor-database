        ...
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-white/10 px-5 py-4">
          <div>
            <div className="mb-1.5 inline-flex items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-[10px] font-medium text-emerald-300">
              <Shield className="h-3 w-3" /> Şifreli tarayıcı kasası
            </div>
            <h2 className="text-lg font-semibold text-white">{isEditing ? `${initialServer?.name} bağlantısını düzenle` : `${selectedEngine.label} sunucusu ekle`}</h2>
            <p className="mt-1 text-xs text-zinc-500">Profil bilgileri şifreli kasada tutulur; bağlantı yalnızca doğrulanmış API isteğinde açılır.</p>
          </div>
          <Button type="button" variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={onClose}><X className="h-4 w-4" /></Button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="min-h-0 flex-1 overflow-y-auto p-5">
          <div className="mb-4 grid gap-3 rounded-xl border border-white/10 bg-white/[0.025] p-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input value={values.host} onChange={event => updateValue('host', event.target.value)} placeholder="192.168.50.25" className={INPUT_CLASS} required />
            </div>
            <div>
              <label className={LABEL_CLASS}>Port</label>
              <Input value={values.port} onChange={event => updateValue('port', event.target.value)} inputMode="numeric" className={INPUT_CLASS} required />
            </div>
            <div>
              <label className={LABEL_CLASS}>Kullanıcı adı</label>
              <Input value={values.username} onChange={event => updateValue('username', event.target.value)} autoComplete="off" className={INPUT_CLASS} required />
            </div>
            <div>
              <label className={LABEL_CLASS}>Parola</label>
              <Input value={values.password} onChange={event => updateValue('password', event.target.value)} type="password" autoComplete="new-password" className={INPUT_CLASS} required />
            </div>
            <div>
              <label className={LABEL_CLASS}>Varsayılan veritabanı</label>
              <Input value={values.databaseName} onChange={event => updateValue('databaseName', event.target.value)} placeholder="coreor_proxy" className={INPUT_CLASS} />
            </div>
            <div>
              <label className={LABEL_CLASS}>Bağlantı zaman aşımı (ms)</label>
              <Input value={values.connectionTimeoutMs} onChange={event => updateValue('connectionTimeoutMs', event.target.value)} inputMode="numeric" className={INPUT_CLASS} />
            </div>
          </div>

          <div className="grid gap-3 rounded-xl border border-white/10 bg-white/[0.025] p-4 sm:grid-cols-3">
            <div>
              <label className={LABEL_CLASS}>Motor</label>
              <select value={values.databaseType} onChange={event => {
                const engine = event.target.value as DatabaseEngine;
                updateValue('databaseType', engine);
                updateValue('version', engine === 'mariadb' ? '12.3' : '8.4');
              }} className={`${INPUT_CLASS} w-full rounded-md`}>
                {ENGINE_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
              </select>
            </div>
            <div>
              <label className={LABEL_CLASS}>Sürüm profili</label>
            ...
            <div>
              <label className={LABEL_CLASS}>TLS</label>
              <select value={values.sslMode} onChange={event => updateValue('sslMode', event.target.value as DatabaseSslMode)} className={`${INPUT_CLASS} w-full rounded-md`}>
                <option value="required">Sertifikayı doğrula</option>
                <option value="preferred">Tercih et / esnek</option>
          ...
          <div className="mt-5 flex items-center justify-between border-t border-white/10 pt-4">
            <div className="flex items-center gap-2 text-[10px] text-zinc-600"><Database className="h-3.5 w-3.5" /> MySQL / MariaDB</div>
            <div className="flex items-center gap-2">
              <Button type="button" variant="outline" size="sm" className="h-8 gap-1.5" onClick={handleTest} disabled={testing || submitting}>
                {testing ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Server className="h-3.5 w-3.5" />} Bağlantıyı test et
              </Button>
              <Button type="submit" size="sm" className="h-8" disabled={submitting || testing}>
                {submitting && <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />}{isEditing ? 'Değişiklikleri kaydet' : 'Sunucuyu ekle'}
              </Button>
            </div>
          </div>
        </form>
