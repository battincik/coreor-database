...
export function ServerCreateModal({ open, onClose, onSubmit, initialServer, onUpdate }: ServerCreateModalProps) {
  ...
  const supportsPoolConfiguration = selectedEngine.family === 'mysql';
    ...
    <div className="relative z-10 flex h-[calc(100dvh-12px)] max-h-[860px] w-[calc(100vw-12px)] max-w-[1240px] min-h-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-zinc-950 shadow-2xl sm:h-[calc(100dvh-24px)] sm:w-[calc(100vw-24px)]">
      ...
      <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
        <div className="coreor-scrollbar min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 sm:p-4">
          ...
          <div className="grid min-w-0 grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1.18fr)_minmax(300px,.82fr)]">
            ...
            <div className="grid content-start gap-3">
              ...
              <FormSection icon={Network} title={t('server.connectionPolicy')} description={t('server.connectionPolicyDescription')}>
                <div className={`grid gap-2 ${supportsPoolConfiguration ? 'sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3' : 'sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2'}`}><div><label className={LABEL_CLASS}>{t('server.tls')}</label><SearchSelect value={values.sslMode} options={tlsOptions} onValueChange={sslMode => updateValue('sslMode', sslMode)} searchPlaceholder={t('server.searchTls')} dropdownMinWidth={380} /></div><div><label className={LABEL_CLASS}>{t('server.timeout')}</label><SearchSelect value={values.connectionTimeoutMs} options={timeoutOptions} onValueChange={connectionTimeoutMs => updateValue('connectionTimeoutMs', connectionTimeoutMs)} searchPlaceholder={t('server.searchTimeout')} dropdownMinWidth={360} /></div>{supportsPoolConfiguration && <div><label className={LABEL_CLASS}>{t('server.poolConnections')}</label><Input type="number" min={1} max={poolConnectionLimit} value={values.poolMaxConnections} onChange={event => updateValue('poolMaxConnections', event.target.value)} onBlur={() => { const raw = Number(values.poolMaxConnections || 6); updateValue('poolMaxConnections', String(Math.min(poolConnectionLimit, Math.max(1, Number.isFinite(raw) ? Math.trunc(raw) : 6)))); }} className={INPUT_CLASS} /></div>}</div>
                {supportsPoolConfiguration && <div className="mt-2 text-[8px] leading-4 text-zinc-600">{discoveredConnectionLimit ? t('server.poolConnectionsDetected', { serverMax: discoveredConnectionLimit, appMax: poolConnectionLimit }) : t('server.poolConnectionsBeforeTest', { appMax: 32 })}</div>}
          ...
          <div className="mt-3 rounded-xl border border-zinc-800 bg-black/15 p-2.5"><div className="mb-2 text-[8px] font-semibold uppercase tracking-[0.14em] text-zinc-600">{t('server.connectionSummary')}</div><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-7"><SummaryItem icon={Server} label={t('server.profile')} value={values.name || t('server.unnamedConnection')} /><SummaryItem icon={Network} label={t('server.target')} value={`${values.host || 'host'}:${values.port || selectedEngine.defaultPort}`} mono /><SummaryItem icon={Database} label={t('server.typeAndVersion')} value={`${selectedEngine.label} ${values.version}`} /><SummaryItem icon={Building2} label={t('server.workspace')} value={organizationLabel} /><SummaryItem icon={Clock3} label={t('server.policy')} value={`${tlsLabel} • ${timeoutLabel}`} />{supportsPoolConfiguration && <SummaryItem icon={Network} label={t('server.poolConnections')} value={`${values.poolMaxConnections || 6} / ${poolConnectionLimit}`} mono />}<SummaryItem icon={LockKeyhole} label={t('server.access')} value={values.readOnly ? t('server.readOnlyAccess') : t('server.readWriteAccess')} /></div></div>
          {error && <div className="mt-3 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-[10px] text-red-300">{error}</div>}
          {testResult && <div className="mt-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-[10px] text-emerald-300"><div className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5" />{testResult}</div><div className="mt-1 pl-5 text-[8px] text-emerald-200/70">{supportsPoolConfiguration ? (discoveredConnectionLimit ? t('server.connectionCapacityDetail', { serverMax: discoveredConnectionLimit, selected: values.poolMaxConnections || 6, cap: poolConnectionLimit }) : t('server.connectionCapacityUnavailable')) : (discoveredConnectionLimit ? t('server.serverConnectionCapacity', { serverMax: discoveredConnectionLimit }) : t('server.connectionCapacityUnavailable'))}</div></div>}
        </div>
        ...
        <footer className="z-10 flex shrink-0 flex-col gap-2 border-t border-white/10 bg-zinc-950 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between sm:px-4">
          <div className="flex min-w-0 items-center gap-2 text-[8px] text-zinc-600"><Sparkles className="h-3 w-3 shrink-0 text-cyan-400" /><span className="truncate">{selectedEngine.label} {values.version} • {values.host || 'host'}:{values.port || selectedEngine.defaultPort}{supportsPoolConfiguration ? ` • Pool ${values.poolMaxConnections || 6}` : ''} • {values.readOnly ? 'READ ONLY' : 'READ/WRITE'} • {organizationLabel}</span></div>
