...
function parseValue(source: string, column: TableColumnInfo): unknown {
  ...
  if (/^(JSON|JSONB)$/.test(type)) {
    if (!source.trim()) throw new Error(`${column.Field} geçerli JSON bekliyor.`);
    return JSON.parse(source);
  }
  if (/^(TINYINT|SMALLINT|MEDIUMINT|INT|INTEGER|BIGINT|DECIMAL|NUMERIC|FLOAT|DOUBLE|REAL|BIT|YEAR|MONEY)$/.test(type)) {
    if (!source.trim()) throw new Error(`${column.Field} için sayısal değer girin.`);
    const numeric = Number(source);
    if (!Number.isFinite(numeric)) throw new Error(`${column.Field} geçerli bir sayı bekliyor.`);
    return numeric;
  }
  if (/^(BOOL|BOOLEAN)$/.test(type)) {
    ...
    throw new Error(`${column.Field} için true/false veya 1/0 kullanın.`);
  }
  return source;
}
...
export function TableRowInsertModal({
  ...
  const save = async () => {
    if (!accountId) { setError('Yerel çalışma alanı hazır değil.'); return; }
    ...
    try {
      ...
      setError(failure instanceof Error ? failure.message : 'Satır eklenemedi.');
    } finally {
      setBusy(false);
    }
  };
      ...
      <div className="relative z-10 flex max-h-[86vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-cyan-500/20 bg-zinc-950 shadow-2xl">
        <header className="flex shrink-0 items-start gap-3 border-b border-zinc-800 bg-cyan-500/[0.035] px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-sm font-semibold">Yeni satır ekle</h3>
            <p className="mt-1 text-[10px] text-zinc-500">{databaseName}.{tableName} • INSERT sorgusu native katmanda güvenli identifier/literal kurallarıyla oluşturulur.</p>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          <div className="space-y-2">
            ...
            {!columns.length && <div className="rounded-xl border border-zinc-800 p-6 text-center text-xs text-zinc-500">Bütün kolonlar otomatik üretildiği için sunucunun varsayılan değerleriyle satır eklenecek.</div>}
          </div>
