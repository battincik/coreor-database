  ...
  return createPortal(<div className="fixed inset-0 z-[620] flex items-center justify-center p-4">
    ...
    <div className="relative z-10 w-full max-w-2xl overflow-hidden rounded-2xl border border-red-500/25 bg-zinc-950 shadow-2xl">
      ...
      <div className="space-y-4 p-5">
        <div className="overflow-hidden rounded-xl border border-zinc-800 bg-black/30"><div className="flex h-8 items-center gap-2 border-b border-zinc-800 px-3 text-[9px] text-zinc-600"><Code2 className="h-3.5 w-3.5 text-cyan-400" />Çalıştırılacak SQL</div><SqlCodeBlock sql={action.sql} className="max-h-52 p-4" /></div>
        <div><label className="mb-2 block text-[10px] text-zinc-400">Onaylamak için <span className="font-mono font-semibold text-red-300">{action.expectedText}</span> yazın.</label><Input autoFocus value={value} onChange={event => { setValue(event.target.value); setError(null); }} className="h-10 border-red-500/20 bg-black/30 font-mono text-xs focus-visible:ring-red-500/30" placeholder={action.expectedText} /></div>
        {error && <div className="rounded-lg border border-red-500/25 bg-red-500/10 px-3 py-2 text-[10px] text-red-300">{error}</div>}
      </div>
      <footer className="flex items-center justify-end gap-2 border-t border-zinc-800 px-5 py-3"><Button variant="ghost" size="sm" disabled={busy} onClick={onClose}>İptal</Button><Button variant="destructive" size="sm" disabled={!matches || busy} onClick={async () => { setBusy(true); setError(null); try { await action.onConfirm(); onClose(); } catch (failure) { setError(failure instanceof Error ? failure.message : 'İşlem tamamlanamadı.'); } finally { setBusy(false); } }}>{busy && <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />}{action.confirmLabel}</Button></footer>
    </div>
  </div>, document.body);
}
