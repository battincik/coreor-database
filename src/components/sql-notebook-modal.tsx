...
function createCell(type: NotebookCellType): NotebookCell {
  return {
    id: createId('cell'),
    type,
    content: type === 'sql' ? DEFAULT_SQL : '# Yeni not\n\nAçıklamanızı Markdown olarak yazın.',
...
export function SqlNotebookModal({ open, onClose, serverId, accountId, databases, selectedDatabase }: SqlNotebookModalProps) {
  ...
  const openCellMenu = (event: React.MouseEvent, cell: NotebookCell, index: number) => openContextMenu(event, [
    { id: 'run', label: 'Hücreyi çalıştır', icon: Play, disabled: cell.type !== 'sql' || !cell.content.trim() || cell.isRunning || !serverId || !accountId, onSelect: () => void runCell(cell.id) },
    { id: 'copy', label: 'Hücre içeriğini kopyala', icon: Copy, onSelect: () => navigator.clipboard.writeText(cell.content) },
    { id: 'duplicate', label: 'Hücreyi çoğalt', icon: Copy, onSelect: () => duplicateCell(cell.id) },
    { id: 'sep-move', separator: true },
    { id: 'move-up', label: 'Yukarı taşı', icon: ArrowUp, disabled: index === 0, onSelect: () => moveCell(cell.id, -1) },
    { id: 'move-down', label: 'Aşağı taşı', icon: ArrowDown, disabled: !activeDocument || index === activeDocument.cells.length - 1, onSelect: () => moveCell(cell.id, 1) },
    ...
    { id: 'clear-result', label: 'Hücre sonucunu temizle', icon: X, disabled: !cell.result && !cell.error, onSelect: () => updateCell(cell.id, { result: null, error: null, executedAt: null }) },
    { id: 'sep-danger', separator: true },
    { id: 'delete', label: 'Hücreyi sil', icon: Trash2, danger: true, disabled: !activeDocument || activeDocument.cells.length <= 1, onSelect: () => removeCell(cell.id) }
