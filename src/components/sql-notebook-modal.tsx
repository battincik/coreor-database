...
export function SqlNotebookModal({ open, onClose, serverId, accountId, databases, selectedDatabase }: SqlNotebookModalProps) {
  ...
  const deleteDocument = () => {
    if (!activeDocument || !window.confirm(`${activeDocument.title} notebook’u silinsin mi?`)) return;
    const next = documents.filter(document => document.id !== activeDocument.id);
  ...
  const importDocument = async (file: File) => {
    try {
      ...
      setDocuments(previous => [document, ...previous].slice(0, MAX_DOCUMENTS));
      setActiveDocumentId(document.id);
    } catch (failure) { window.alert(failure instanceof Error ? failure.message : 'Notebook içe aktarılamadı.'); }
  };
