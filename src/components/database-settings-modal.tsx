...
import {
  Accessibility, Building2, Code2, Database, Github, KeyRound, Laptop, Palette,
  RefreshCw, RotateCcw, Server, Settings2, ShieldCheck, SlidersHorizontal,
...
function SettingRow({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <div className="grid min-h-16 items-center gap-3 border-b border-zinc-800/70 py-3 last:border-0 md:grid-cols-[minmax(180px,1fr)_minmax(320px,520px)]"><div><div className="text-[11px] font-medium text-zinc-200">{title}</div><div className="mt-1 text-[9px] leading-4 text-zinc-600">{description}</div></div><div className="min-w-0 w-full">{children}</div></div>;
}
