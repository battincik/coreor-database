...
import type { ActivityLogLimit, AppFontFamily, AppThemeName, PerformanceRefreshSeconds, SyntaxThemeName } from '@/lib/appPreferences';
...
import { readLocalVaultStatus, sqlLogPath, type LocalVaultStatus } from '@/lib/desktopClient';
...
const TABS: Array<{ id: DatabaseSettingsTab; labelKey: string; descriptionKey: string; icon: SettingsIcon }> = [
  ...
  { id:'query-log', labelKey:'settingsCatalog.tabs.queryLog', descriptionKey:'settingsCatalog.tabs.queryLogDescription', icon:Terminal },
...
export function DatabaseSettingsModal({ open, onClose, initialTab = 'account' }: DatabaseSettingsModalProps) {
  ...
  const [activityLogPath, setActivityLogPath] = useState<string | null>(null);
  ...
  const activityLogLimitOptions=useMemo<SearchSelectOption<ActivityLogLimit>[]>(()=>[500,1000,2500,5000,10000].map(value=>({value:value as ActivityLogLimit,label:t('settingsCatalog.queryLog.entryCount',{count:formatNumber(value)}),badge:value===1000?t('common.default'):undefined})),[t,formatNumber]);
  ...
  useEffect(() => {
    if (!open || (tab !== 'query-log' && !preferences.activityLogPersistToDisk)) return;
    void sqlLogPath()
      .then(path => setActivityLogPath(path))
      .catch(() => setActivityLogPath(null));
  }, [open, tab, preferences.activityLogPersistToDisk]);
    ...
    <div className="fixed inset-0 z-[340] flex items-center justify-center p-2 sm:p-3">
      ...
      <div className="relative z-10 grid h-[calc(100dvh-16px)] max-h-[880px] w-[calc(100vw-16px)] max-w-[1440px] grid-cols-[clamp(170px,20vw,235px)_minmax(0,1fr)] overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950 shadow-2xl sm:h-[calc(100dvh-24px)] sm:w-[calc(100vw-24px)]">
        <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden border-r border-zinc-800 bg-black/25">
          <div className="border-b border-zinc-800 p-4"><div className="flex items-center gap-2"><Settings2 className="h-4 w-4 text-cyan-400" /><div><div className="text-xs font-semibold">{t('app.settings')}</div><div className="text-[8px] text-zinc-600">{APP_VERSION_LABEL}</div></div></div></div>
  ...
  ['commandPalette',t('settingsCatalog.shortcuts.commandPalette')],['newQuery',t('settingsCatalog.shortcuts.newQuery')],['runQuery',t('settingsCatalog.shortcuts.runQuery')],['formatSql',t('settingsCatalog.shortcuts.formatSql')],['refresh',t('common.refresh')],['insertRow',t('tableData.addRow')],['settings',t('app.settings')],['find',t('settingsCatalog.shortcuts.find')],['closeTab',t('settingsCatalog.shortcuts.closeTab')],['duplicateTab',t('settingsCatalog.shortcuts.duplicateTab')],['language',t('settingsCatalog.shortcuts.language')],['backupCenter',t('settingsCatalog.shortcuts.backupCenter')],['automationCenter',t('settingsCatalog.shortcuts.automationCenter')]
            ...
            {tab === 'query-log' && <Section icon={Terminal} title={t('settingsCatalog.queryLog.title')} description={t('settingsCatalog.queryLog.description')}><Row title={t('settingsCatalog.queryLog.limit')} description={t('settingsCatalog.queryLog.limitDescription')}><SearchSelect value={preferences.activityLogLimit} options={activityLogLimitOptions} onValueChange={activityLogLimit => setPreferences({ activityLogLimit })} /></Row><Row title={t('settingsCatalog.queryLog.disk')} description={t('settingsCatalog.queryLog.diskDescription')}><div className="space-y-2"><CoreorSwitch checked={preferences.activityLogPersistToDisk} onCheckedChange={activityLogPersistToDisk => setPreferences({ activityLogPersistToDisk })} label={t('settingsCatalog.queryLog.diskToggle')} />{activityLogPath&&<div className="break-all rounded-lg border border-zinc-800 bg-black/25 px-3 py-2 font-mono text-[9px] text-zinc-500">{activityLogPath}</div>}</div></Row><Row title={t('settingsCatalog.queryLog.errors')} description={t('settingsCatalog.queryLog.errorsDescription')}><CoreorSwitch checked={preferences.activityLogErrors} onCheckedChange={activityLogErrors => setPreferences({ activityLogErrors })} label={t('settingsCatalog.queryLog.errorsToggle')} /></Row><Row title={t('settingsCatalog.queryLog.userQueries')} description={t('settingsCatalog.queryLog.userQueriesDescription')}><CoreorSwitch checked={preferences.activityLogUserQueries} onCheckedChange={activityLogUserQueries => setPreferences({ activityLogUserQueries })} label={t('settingsCatalog.queryLog.userQueriesToggle')} /></Row><Row title={t('settingsCatalog.queryLog.internalQueries')} description={t('settingsCatalog.queryLog.internalQueriesDescription')}><CoreorSwitch checked={preferences.activityLogInternalQueries} onCheckedChange={activityLogInternalQueries => setPreferences({ activityLogInternalQueries })} label={t('settingsCatalog.queryLog.internalQueriesToggle')} /></Row><Row title={t('settingsCatalog.queryLog.info')} description={t('settingsCatalog.queryLog.infoDescription')}><CoreorSwitch checked={preferences.activityLogInfo} onCheckedChange={activityLogInfo => setPreferences({ activityLogInfo })} label={t('settingsCatalog.queryLog.infoToggle')} /></Row></Section>}
