      ...
      <div className="relative z-10 grid h-[min(880px,calc(100dvh-24px))] w-[min(1440px,calc(100vw-24px))] grid-cols-[255px_minmax(0,1fr)] overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950 shadow-2xl">
        ...
        <main className="flex min-h-0 flex-col">
          ...
          <div className="coreor-table-scroll min-h-0 flex-1 overflow-auto p-5">
            ...
            ].map(item => { const Icon = item.icon; return <div key={item.label} className="rounded-xl border border-zinc-800 bg-black/20 p-3"><div className="flex items-center gap-2 text-[8px] text-zinc-600"><Icon className="h-3 w-3" />{item.label}</div><div className="mt-1 truncate text-[10px]">{item.value}</div></div>; })}</div></section><Section icon={KeyRound} title="Hesap ve kasa" description="Oturum, şifreli profiller ve ekip kapsamı."><Row title="Şifreli sunucu kasası" description="Bu cihazda AES tabanlı saklanan bağlantı profilleri."><div className="rounded-xl border border-cyan-500/20 bg-cyan-500/[0.05] px-3 py-2 text-[10px] text-cyan-300">{servers.length} profil • {totalDatabases} veritabanı • {totalTables} tablo</div></Row><Row title="Organizasyonlar" description="Ekip rolleri ve veritabanı bağları."><Button variant="outline" className="w-full justify-between" onClick={() => setTab('organizations')}>Organizasyonları yönet<Building2 className="h-4 w-4" /></Button></Row></Section></div>}
            ...
            ].map(item => { const Icon = item.icon; return <div key={item.label} className="rounded-2xl border border-zinc-800 p-4"><div className="flex items-center gap-2 text-[9px] text-zinc-600"><Icon className="h-4 w-4 text-cyan-400" />{item.label}</div><div className="mt-2 text-2xl font-semibold">{item.value}</div></div>; })}</div><Section icon={Server} title="Bağlantı profilleri" description="Motor, host, katalog ve erişim politikaları."><div className="space-y-2">{servers.map(server => <div key={server.id} className="flex items-center gap-3 rounded-xl border border-zinc-800 p-3"><Server className="h-4 w-4 text-emerald-400" /><div className="min-w-0 flex-1"><div className="text-[11px] font-medium">{server.name}</div><div className="font-mono text-[9px] text-zinc-600">{server.host}:{server.port}</div></div>{server.readOnly && <span className="rounded bg-amber-500/10 px-2 py-1 text-[8px] text-amber-300">READ ONLY</span>}<span className="text-[9px] text-zinc-500">{databaseEngineLabel(server.databaseType)} {server.version}</span></div>)}</div><Button variant="outline" size="sm" className="mt-4" disabled={isServersLoading} onClick={() => void loadServers()}><RefreshCw className={`mr-2 h-3.5 w-3.5 ${isServersLoading ? 'animate-spin' : ''}`} />Kasayı yenile</Button></Section></div>}
