...
export async function readEncryptedServerProfiles(_accountId?: string) {
  const config = await readDesktopConfig();
  return cloneProfiles(config.connections as DatabaseServerConfig[]);
}

export async function writeEncryptedServerProfiles(_accountId: string, servers: DatabaseServerConfig[]) {
  const config = await readDesktopConfig();
