...
const releases = [
  { version: '1.9.0', pr: 'Yeni PR', date: '26 Temmuz 2026', title: 'Advanced Database Workbench', tone: 'from-cyan-500/20 to-purple-500/10', items: ['MySQL kullanıcı, rol ve GRANT/REVOKE yönetimi', 'Process listesi, metadata lock ve deadlock merkezi', 'Sürüklenebilir ER/flow şeması ve kolondan foreign key oluşturma', 'CSV, JSON ve SQL içe aktarma; CSV, JSON ve INSERT SQL dışa aktarma', 'SQL autocomplete, formatter, snippet, geçmiş ve favoriler', '7 uygulama teması, 6 syntax teması, erişilebilirlik ve gelişmiş ayarlar', 'Kalıcı ve sürüklenebilir sidebar genişliği'] },
...
export default function WhatsNewPage() {
  return (
    <SettingsPageShell activeTab="whatsNew" icon={Sparkles} eyebrow="Ürün günlüğü" title="Yenilikler" description="Coreor Database'in ilk şifreli kasa sürümünden gelişmiş veritabanı çalışma alanına kadar bütün önemli değişiklikler.">
      <div className="rounded-xl border bg-card/60 p-4 text-xs text-muted-foreground"><strong className="text-foreground">Sürümleme:</strong> Her birleşen PR ürün sürümünde yeni bir minor sürüm oluşturur. Düzeltme odaklı PR’lar da kullanıcı deneyimi kilometre taşı olarak korunur.</div>
