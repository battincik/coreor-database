...
async fn table_info(c:&Connection,p:&Map<String,Value>,max:usize)->Result<Value,String>{
    ...
    let sql=if is_pg(&c.engine){format!("SELECT column_name AS \"Field\",data_type AS \"Type\",is_nullable AS \"Null\",'' AS \"Key\",column_default AS \"Default\",'' AS \"Extra\",'' AS \"Comment\",NULL AS \"Collation\",ordinal_position AS \"Ordinal_position\",data_type AS \"Data_type\",character_maximum_length AS \"Character_maximum_length\",numeric_precision AS \"Numeric_precision\",numeric_scale AS \"Numeric_scale\",datetime_precision AS \"Datetime_precision\",NULL AS \"Character_set_name\",'' AS \"Generation_expression\" FROM information_schema.columns WHERE table_schema='public' AND table_name='{}' ORDER BY ordinal_position",table.replace("'","''"))}else{format!("SELECT c.name AS Field,TYPE_NAME(c.user_type_id) AS Type,CASE WHEN c.is_nullable=1 THEN 'YES' ELSE 'NO' END AS [Null],'' AS [Key],OBJECT_DEFINITION(c.default_object_id) AS [Default],CASE WHEN c.is_identity=1 THEN 'identity' ELSE '' END AS Extra,'' AS Comment,NULL AS Collation,c.column_id AS Ordinal_position,TYPE_NAME(c.user_type_id) AS Data_type,c.max_length AS Character_maximum_length,c.precision AS Numeric_precision,c.scale AS Numeric_scale,NULL AS Datetime_precision,NULL AS Character_set_name,'' AS Generation_expression FROM sys.columns c WHERE c.object_id=OBJECT_ID(N'dbo.{}') ORDER BY c.column_id",table.replace("'","''"))};
    let columns=rows_of(&execute_sql(c,&sql,Some(db),max).await?);
    Ok(json!({"table":{"name":table,"comment":"","engine":c.engine,"collation":null,"charset":null,"autoIncrement":null,"rowFormat":null,"tableType":"BASE TABLE","createTime":null,"updateTime":null},"columns":columns,"indexes":[],"foreignKeys":[],"checkConstraints":[],"partitions":[],"createSQL":"","_meta":{"statements":[{"label":"Tablo yapısı","sql":sql}]}}))
}
...
async fn alter_table(c:&Connection,p:&Map<String,Value>,max:usize)->Result<Value,String>{
    ...
    let sql=match kind{
      "table-options"=>{if let Some(n)=m.get("name").and_then(Value::as_str){if !n.trim().is_empty()&&n!=table{let s=if is_mssql(&c.engine){format!("EXEC sp_rename N'dbo.{}', N'{}'",table.replace("'","''"),n.replace("'","''"))}else{format!("ALTER TABLE {} RENAME TO {}",qt,ident(n,&c.engine)?)};execute_sql(c,&s,Some(db),1).await?;table=n.to_string();} } if let Some(comment)=m.get("comment").and_then(Value::as_str){if is_pg(&c.engine){format!("COMMENT ON TABLE {} IS {}",qualified(db,&table,&c.engine)?,literal(&json!(comment),&c.engine))}else if is_mssql(&c.engine){String::new()}else{format!("ALTER TABLE {} COMMENT={}",qualified(db,&table,&c.engine)?,literal(&json!(comment),&c.engine))}}else{String::new()}},
      ...
      "add-index"=>{let ix=m.get("index").and_then(Value::as_object).ok_or("İndeks eksik.")?;let cols=ix.get("columns").and_then(Value::as_array).ok_or("İndeks kolonu eksik.")?.iter().filter_map(|x|x.get("name").and_then(Value::as_str)).map(|x|ident(x,&c.engine)).collect::<Result<Vec<_>,_>>()?.join(", ");let kind=ix.get("kind").and_then(Value::as_str).unwrap_or("INDEX");let name=ix.get("name").and_then(Value::as_str).unwrap_or("idx_coreor");if kind=="PRIMARY"{format!("ALTER TABLE {} ADD PRIMARY KEY ({})",qt,cols)}else{format!("CREATE {} INDEX {} ON {} ({})",if kind=="UNIQUE"{"UNIQUE"}else{""},ident(name,&c.engine)?,qt,cols)}},
...
async fn workbench(c:&Connection,action:&str,p:&Map<String,Value>,max:usize)->Result<Value,String>{
 match action{
 "process-list"=>{let sql=if is_pg(&c.engine){"SELECT pid AS id,usename AS user,COALESCE(client_addr::text,'') AS host,datname AS database,state AS command,EXTRACT(EPOCH FROM(now()-query_start))::bigint AS seconds,wait_event AS state,query AS info FROM pg_stat_activity ORDER BY query_start NULLS LAST"}else if is_mssql(&c.engine){"SELECT r.session_id AS id,s.login_name AS [user],s.host_name AS host,DB_NAME(r.database_id) AS [database],r.command,DATEDIFF(SECOND,r.start_time,SYSDATETIME()) AS seconds,r.status AS state,t.text AS info FROM sys.dm_exec_requests r JOIN sys.dm_exec_sessions s ON s.session_id=r.session_id CROSS APPLY sys.dm_exec_sql_text(r.sql_handle)t"}else{"SELECT ID AS id,USER AS user,HOST AS host,DB AS database,COMMAND AS command,TIME AS seconds,STATE AS state,INFO AS info FROM information_schema.PROCESSLIST ORDER BY TIME DESC"};let r=execute_sql(c,sql,None,max).await?;Ok(json!({"processes":r["rows"],"locks":[],"deadlockText":null,"currentConnectionId":null}))},
 ...
 "user-save"|"privilege-change"=>Err(format!("{} işlemi bu motor için native masaüstü katmanında güvenli bir genel forma dönüştürülemedi; motorun SQL editörünü kullanın.",action)),
