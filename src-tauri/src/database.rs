...
pub async fn execute_action(request:DatabaseRequest,max_rows:usize,max_page:usize)->Result<Value,String>{
 ...
 match action.as_str(){
 "test"=>{let sql=if is_mssql(&c.engine){"SELECT @@VERSION AS version,DB_NAME() AS databaseName,SUSER_SNAME() AS currentUser,@@MAX_CONNECTIONS AS maxConnections"}else if is_pg(&c.engine){"SELECT version() AS version,current_database() AS \"databaseName\",current_user AS \"currentUser\",current_setting('max_connections')::bigint AS \"maxConnections\""}else{"SELECT VERSION() AS version,DATABASE() AS databaseName,CURRENT_USER() AS currentUser,@@GLOBAL.max_connections AS maxConnections"};let r=execute_sql(&c,sql,p.get("database").and_then(Value::as_str),1).await?;let row=rows_of(&r).into_iter().next().unwrap_or(json!({}));Ok(json!({"connection":{"version":row.get("version"),"databaseName":row.get("databaseName"),"currentUser":row.get("currentUser"),"maxConnections":row.get("maxConnections")},"_meta":{"statements":[{"label":"Bağlantı testi","sql":sql}]}}))},
